...
uint64_t find_max(char *line, size_t *counters, size_t len,
                  size_t min_distance_to_end) {
  size_t idx = 0;
  uint64_t max = 0;
...
uint64_t largest_jolt_for_line(char *line, size_t digitis) {
  ...
  size_t *counters = create_counters(line);
  uint64_t value = 0;
  for (size_t idx = 0; idx < digitis; idx++) {
    size_t ele_idx = find_max(line, counters, len, digitis - idx);
